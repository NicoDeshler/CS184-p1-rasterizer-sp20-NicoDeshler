...
void RasterizerImp::fill_pixel(size_t x, size_t y, Color c) {
  // TODO: Task 2: You might need to use this function to fix points and lines (such as the black rectangle border in test4.svg)
...
void RasterizerImp::fill_supersample(size_t x, size_t y, size_t s, Color c) {
  // TODO: Task 2: You may want to implement this function. Hint: our solution uses one line
    ...
    supersample_buffer[sample_rate * (sy * width + sx) + s] = c;
    return;
}
            ...
            for (int j = 0; j < sr; j++) {
                ...
                for (int i = 0; i < sr; i++) {
                    ...
                    if ((dot(p - p1, n0) >= 0) && (dot(p - p2, n1) >= 0) && (dot(p - p0, n2) >= 0)) {
                        ...
                        // Get UV space coordinates using berrycentric coords
...
void RasterizerImp::resolve_to_framebuffer() {
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            ...
            for (int s = 0; s < sample_rate; s++) {
                avg_color += supersample_buffer[sample_rate * (y * width + x) + s];
